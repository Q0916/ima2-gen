...
승인 후 head `99df83ea`에서 체크 10개가 전부 green이 됐다. 가장 오래 걸린 것은
PR frontend checks로 20분 56초. 즉 `UNSTABLE`은 처음부터 코드 문제가 아니었고,
fork PR 워크플로 승인이라는 유지보수자 쪽 미처리가 전부였다. 현재 `main`(`22e7c1ab`)
기준으로 ahead=2 behind=11.

### 리뷰 게시

`gh pr review 229 --request-changes`로 게시. 게시 전에 fact-check 리뷰어를 한 번 더
돌렸고 near-pass와 함께 6건을 잡아내 전부 고친 뒤 올렸다. 외부 기여자에게 나가는
글이라 수치 하나가 틀리면 신뢰를 잃는다.

| 수정 | 내용 |
|---|---|
| reasoning 표 누락 | `lib/providerOptions.ts:138-140` 행 추가 |
| 잘못된 집계 | "12 call sites" → "10개 파일 15개 기본값/폴백 식, 그중 7개가 요청 페이로드 지점" |
| 과장 | "느려지고 비싸진다" → "지연과 비용이 올라갈 수 있다". 근거 없는 "대부분의 사용자" 삭제 |
| 누락된 예외 | `responsesDoctor` 프로브 3개는 설정과 무관하게 `max`를 하드코딩한다는 사실 추가 |
| 로깅 범위 과장 | "모든 요청" → "readiness 통과 후 나가는 `postResponses` 전송" |
| ComfyUI 표현 | "선택할 수 없다" → "명시적으로 선택할 수 없다". 빈 값은 서버 기본값으로 폴백한다 |
| 문서 스윕 누락 | `docs/API.md:335`, `docs/FAQ.md:139`, 로컬라이즈된 README 표, `site/src/pages/{docs,ko/docs}`, `README.md:343`의 허용값에 `max` 추가 |
| CodeRabbit 두 번째 스레드 | `PromptComposerToolbar.tsx:59`는 이 PR 이전부터 있던 동작이라고 명시하고 기여자에게 요구하지 않음 |

### 종결 상태: 미결

리뷰는 남겼지만 PR은 열어 둔다. 030의 무응답 기본 경로는 close였는데, 유지보수자에게
던진 제품 질문이 아직 답을 기다리는 상태에서 외부 기여자의 PR을 닫는 것은 이 작업에서
할 수 있는 가장 불친절한 선택이고, 3.16.1을 내보내는 데 필요하지도 않다.

따라서 goalplan criterion c-7("merged or closed")은 **충족되지 않았다.** 기준을 낮춰
통과시키지 않고 미결로 보고한다. 릴리스는 이 판정과 무관하게 진행한다 — #229의 커밋은
3.16.1에 들어가지 않는다.
